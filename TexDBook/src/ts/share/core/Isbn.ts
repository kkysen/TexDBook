...
type GroupRecord = {
    readonly group: IsbnGroup;
    readonly rest: string;
}

export interface IsbnGroup {
...
const GROUPS: {[code: string]: IsbnGroup} = {
    "0": {
        code: "0",
...
interface MutableIsbn {
    ...
    isIsbn10(): this is Isbn10;
    
    isIsbn13(): this is Isbn13;
    
    asIsbn10(hyphenate: boolean): string | null;
    
    asIsbn13(hyphenate: boolean): string | null;
...
type IsbnBuilder = Partial<MutableIsbn>;

interface MutableIsbn13 extends MutableIsbn {
...
interface MutableIsbn10 extends MutableIsbn {
...
export type Isbn10 = Readonly<MutableIsbn10>;
...
export const Isbn: IsbnClass = (() => {
    ...
    const buildRemainingFields = function(isbn?: IsbnBuilder): MutableIsbn | null {
        if (!isbn) {
            return null;
        }
        ...
        const parts13: string[] = [prefix, ...parts, check13];
        ...
        merge(isbn, {
            ...
            asIsbn13(hyphenate: boolean = false): string {
                return hyphenate ? isbn13Hyphenated : isbn13;
            },
        });
        
        if (prefix === isbn10Prefix) {
            const parts10: string[] = [...parts, check10];
    ...
    return {
        ...
        parse(isbnString: string, _groups: IsbnGroup[] = Object.values(GROUPS)): Isbn | null {
            ...
            const splitAndBuild10 = function(isbn10: string): IsbnBuilder | null {
                const record: GroupRecord | null = getGroupRecord(isbn10);
                if (!record) {
                    return null;
                }
                const {group, rest}: GroupRecord = record;
            ...
            const splitAndBuild = function(isbn?: string): IsbnBuilder | null {
                return !isbn
                    ? null
                    : isbn.length === 13
