...
const navLinks = views.map((view, i) => React.createElement("li", { key: view.name },
    React.createElement(react_router_dom_1.NavLink, { to: "/" + view.name }, view.name)));
const routes = views.map((view, i) => React.createElement(react_router_1.Route, { key: view.name, path: "/" + view.name, component: view }));
class Main extends react_1.Component {
    render() {
        return (React.createElement(react_router_dom_1.HashRouter, null,
            React.createElement("div", null,
                React.createElement("h1", null, "TexDBook"),
                React.createElement("ul", { className: "header" }, navLinks),
